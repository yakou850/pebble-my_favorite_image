...
void netdownload_receive(DictionaryIterator *iter, void *context) {
	...
	Tuple *url1 = dict_find(iter, KEY_IMAGE_URL1);
	Tuple *url2 = dict_find(iter, KEY_IMAGE_URL2);
	Tuple *url3 = dict_find(iter, KEY_IMAGE_URL3);
	Tuple *url4 = dict_find(iter, KEY_IMAGE_URL4);
	Tuple *url5 = dict_find(iter, KEY_IMAGE_URL5);
